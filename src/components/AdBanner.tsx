...
/**
 * Interstitial Ad:
 * - Native (Android): loads and shows AdMob interstitial, then calls onClose()
 * - Web: immediately calls onClose() — AdSense does NOT allow custom interstitial
 *   overlays with display ads (policy violation). Native AdMob handles its own UI.
 */
export function InterstitialAd({ onClose }: { onClose: () => void }) {
  const isNative = Capacitor.isNativePlatform();

  // Native: load and show AdMob interstitial
  useEffect(() => {
    if (!isNative) return;
    AdMob.loadInterstitial()
      .then(() => AdMob.showInterstitial())
  ...
  // Web: immediately close — no custom interstitial overlay allowed by AdSense
  useEffect(() => {
    if (isNative) return;
    onClose();
  }, []);

  // Nothing to render — AdMob handles its own native UI
  return null;
}