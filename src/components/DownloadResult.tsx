...
// updateLatestHistoryFilename(title, filename); // ← add this
//
//       setState('done');
//       setProgress('');
...
import { Download, Play, Clock, User, CheckCircle2, Pause, PlayCircle, Music } from 'lucide-react';import { Capacitor } from '@capacitor/core';
import Downloader from '../plugins/Downloader';
import type { DownloadResult, DownloadQuality } from '../types';
import PlatformBadge from './PlatformBadge';
import { useToast } from './Toast';
...
//   setPercent(95);
//
//   // If MP3, route through audio extraction endpoint
//   if (isAudio) {
//     const API = import.meta.env.VITE_API_URL || 'https://vid-backend-pr0o.onrender.com';
...
//
//       // Render job — for both video and audio (Instagram/Facebook MP3 uses same render URL)
// //       const isRenderJob = downloadUrl.includes('render-api') && downloadUrl.includes('execute');
//
//       if (isRenderJob) {
//         setProgress(isAudio ? 'Preparing audio...' : 'Starting render job...');
//         setPercent(10);
//
//         const execRes = await fetch(downloadUrl);
//         const text = await execRes.text();
...
//         if (!sseUrl) throw new Error('No SSE URL');
//
//         setProgress(isAudio ? 'Processing audio...' : 'Processing video...');
//         setPercent(20);
//
//         downloadUrl = await new Promise<string>((resolve, reject) => {
//           const eventSource = new EventSource(sseUrl);
//           const timeout = setTimeout(() => {
//             eventSource.close();
//             reject(new Error('Render timeout — try lower quality'));
//           }, 300000);
//
//           eventSource.onmessage = (event) => {
//             try {
...
//                 setPercent(20 + Math.floor((data.progress || 0) * 0.4));
//               } else if (data.status === 'processing') {
//                 setProgress(isAudio ? 'Extracting audio...' : 'Merging audio & video...');
//                 setPercent(65);
//               } else if (data.status === 'uploading_output') {
//                 setProgress('Finalizing...');
//                 setPercent(85);
...
//                 setPercent(90);
//
//                 // For Instagram/Facebook MP3 — route through audio endpoint
//                 if (isAudio) {
//                   const audioUrl = `${API}/api/audio?videoUrl=${encodeURIComponent(data.output.url)}&title=${encodeURIComponent(title)}`;
//                   resolve(audioUrl);
//                 } else {
//                   resolve(data.output.url);
//                 }
...
//
//       if (Capacitor.isNativePlatform()) {
//         setProgress(isAudio ? 'Downloading audio...' : 'Downloading...');
//         setPercent(5);
//
// //         progressListenerRef.current = await Downloader.addListener(
// //           'downloadProgress',
...
//         ? ` / ${(data.total / (1024 * 1024)).toFixed(1)} MB`
//         : ' MB';
//       // Set MB immediately — replaces progress text right away
//       setDownloadedMB(`${dlMB}${totalMB}`);
//       setProgress(`${dlMB}${totalMB}`);
//     }
//     if (data.percent > 0) setPercent(data.percent);
...
//
//
//         await Downloader.download({ url: downloadUrl, filename, mimeType });
//
//         progressListenerRef.current?.remove();
//         progressListenerRef.current = null;
//
...
//
//       setPercent(100);
//       showSuccess(isAudio ? 'Audio saved to Downloads!' : 'Video saved to Downloads!');
//       setState('done');
//       setProgress('');
//       setDownloadedMB('');
//       setTimeout(() => setState('idle'), 3000);
...
//                   ? downloadedMB
//                   : `${percent}% complete`
//                 : isDone ? 'Check Downloads folder'
//                 : quality.resolution ?? quality.label}
//             </p>
<p className={`text-xs mt-0.5 transition-colors ${
  isDone ? 'text-green-500/60'
  : isPaused ? 'text-amber-500/70'
        ...
        <div className="flex items-center gap-2 relative z-10">
          ...
          {isActive && (
            <span className={`text-xs font-bold tabular-nums min-w-[32px] text-right ${
              isPaused ? 'text-amber-400' : 'text-sky-400'
            }`}>
              {percent}%
            </span>
          )}
