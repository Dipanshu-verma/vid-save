  ...
  const saveFile = useCallback(async (file: MediaFile) => {
    ...
    try {
      ...
      if (Capacitor.isNativePlatform() && file.base64) {
        ...
        // Write to Documents first (always accessible)
        await Filesystem.writeFile({
          path: filename,
          data: file.base64,
          directory: Directory.Documents,
        ...
        // Then try to copy to Downloads folder (visible in gallery)
        try {
          const uriResult = await Filesystem.getUri({
            path: filename,
            directory: Directory.Documents,
          ...
          // Clean up temp file from Documents only if copy succeeded
          await Filesystem.deleteFile({ path: filename, directory: Directory.Documents });

          showSuccess('Status saved to Downloads!');
        } catch (copyErr: any) {
          // Copy to Downloads failed — file is still in Documents
          // Don't throw — the file IS saved, just not in Downloads
