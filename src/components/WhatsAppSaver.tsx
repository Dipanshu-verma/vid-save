...
import { MessageCircle, Download, Image, Film, AlertCircle, CheckCircle2, X, Play, RefreshCw, FolderOpen, Shield } from 'lucide-react';
interface MediaFile {
  name: string;
  url: string;
  type: 'video' | 'image';
...
export default function WhatsAppSaver() {
        ...
        try {
          ...
          for (const file of result.files) {
            ...
            try {
              ...
              const dataUrl = `data:${mimeType};base64,${fileData.data}`;
  ...
  const saveFile = useCallback(async (file: MediaFile) => {
    try {
      ...
      const destName = `WA_Status_${Date.now()}.${ext}`;
      await Filesystem.copy({
        from: file.path,
        to: `Downloads/${destName}`,
        directory: Directory.ExternalStorage,
  ...
  const saveAll = useCallback(() => {
    mediaFiles.forEach((f, i) => setTimeout(() => saveFile(f), i * 500));
  }, [mediaFiles, saveFile]);
      ...
      <div className="space-y-4">
        <div className="flex flex-col items-center justify-center py-8 text-center space-y-4">
          ...
          <div className="flex flex-col gap-3 w-full">
            <button
              onClick={loadStatuses}
              className="flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-slate-700 hover:bg-slate-600 text-white text-sm font-medium transition-all active:scale-95"
            >
            ...
            <button
              onClick={() => inputRef.current?.click()}
              className="flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-green-500 hover:bg-green-400 text-white text-sm font-semibold transition-all active:scale-95"
            >
              <FolderOpen className="w-4 h-4" />
              Pick Files Manually
            </button>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => { setMediaFiles([]); setStep('permission'); }}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-700 hover:bg-slate-600 text-slate-300 text-xs font-medium transition-colors"
          >
            <X className="w-3 h-3" />
            Clear
      ...
      <div className="grid grid-cols-2 gap-2.5">
        {mediaFiles.map((file, i) => {
          ...
          return (
            <div
              key={i}
              className={`relative rounded-2xl overflow-hidden bg-slate-800 border transition-all ${isSaved ? 'border-green-500/40' : 'border-slate-700/50'}`}
            >
              ...
              <div className="flex items-center justify-between px-2.5 py-2">
                ...
                <button
                  ...
                  className={`ml-2 p-1.5 rounded-lg transition-all active:scale-90 ${isSaved ? 'bg-green-500/15 text-green-400' : 'bg-sky-500/15 hover:bg-sky-500/25 text-sky-400'}`}
                >
                  {isSaved ? <CheckCircle2 className="w-3.5 h-3.5" /> : <Download className="w-3.5 h-3.5" />}
                </button>
              </div>
            </div>
          );
        ...
        >
          <div className="relative max-w-lg w-full" onClick={e => e.stopPropagation()}>
            ...
            <button
              ...
              className="w-full mt-3 flex items-center justify-center gap-2 py-3 rounded-xl bg-green-500 hover:bg-green-400 text-white font-semibold text-sm transition-all active:scale-95"
            >
